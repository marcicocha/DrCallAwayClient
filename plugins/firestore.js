...
const setDocument = (path, docId, data) => {
  return setDoc(doc(firestore, path, docId), data)
}
...
const deleteDocument = (ref, docId) => {
  return deleteDoc(doc(firestore, ref, docId))
}
...
const userRef = (userId) => {
  return doc(firestore, USERS_PATH, userId)
}
...
// ROOMS
const roomsRef = collection(firestore, ROOMS_PATH)

const roomRef = (roomId) => {
  return doc(firestore, ROOMS_PATH, roomId)
}
...
const messagesRef = (roomId) => {
  return collection(firestore, MESSAGE_PATH(roomId))
}

const messageRef = (roomId, messageId) => {
  return doc(firestore, MESSAGE_PATH(roomId), messageId)
}
